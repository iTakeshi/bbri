class ReviewsController < ApplicationController
  ...
  def create
    ...
    if review.save
      render json: { status: :success, review: review, new_record: :true, part_identifier: review.part.part_identifier, user: review.user.user_name, time: review.created_at.strftime("%Y.%m.%d %H:%M") }
    else
  ...
  def update
    ...
    if review.save
      render json: { status: :success, review: review, new_record: :false, part_identifier: review.part.part_identifier, user: review.user.user_name, time: review.created_at.strftime("%Y.%m.%d %H:%M") }
    else
      render json: { status: :error }
    end
  end
